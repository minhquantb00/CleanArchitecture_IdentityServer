...
namespace IdentityServer.Application.ImplementServices.UseCases
{
    public class UserService : IUserService
    {
        ...
        public async Task<ApiResponse<DataCreateUserResponse>> CreateUserWithTokenAsync(Request_Register registerUser)
        {
            var userExist = await _userManager.FindByEmailAsync(registerUser.Email);
            if (userExist != null)
            {
                return new ApiResponse<DataCreateUserResponse> { IsSuccess = false, StatusCode = 403, Message = "Người dùng đã tồn tại!" };
            }
            ...
            if (result.Succeeded)
            {
                ...
                return new ApiResponse<DataCreateUserResponse> { Response = new DataCreateUserResponse() { User = user, Token = token }, IsSuccess = true, StatusCode = 201, Message = "Tạo người dùng thành công" };

            }
            else
            {
                return new ApiResponse<DataCreateUserResponse> { IsSuccess = false, StatusCode = 500, Message = "User Failed to Create" };

            }
        }

        public async Task<ApiResponse<DataLoginResponse>> GetJwtTokenAsync(ApplicationUser user)
        {
            ...
            return new ApiResponse<DataLoginResponse>
            {
                Response = new DataLoginResponse()
                {
        ...
        public async Task<ApiResponse<LoginOtpResponse>> GetOtpByLoginAsync(Request_Login loginModel)
        {
        ...
        public async Task<ApiResponse<DataLoginResponse>> LoginUserWithJWTokenAsync(string otp, string userName)
        {
        ...
        public async Task<ApiResponse<DataLoginResponse>> RenewAccessTokenAsync(DataLoginResponse tokens)
        {
